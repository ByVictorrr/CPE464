...
void TCPServer::processClient(int clientSocket){
    ...
    pkt_len = full_recv(clientSocket, recvBuff);
    ...
    case CHECK_HANDLE:
    {
        ...
        safe_send(client, transBuff, 3, 0);
    }
        break;
    case BROADCAST:
    {
    ...
    case MULTICAST:
    {
        ...
        for(const auto &dest: parser.getDestHandles())
            for(const auto &client: this->clients)
                if(dest.compare(client.first) != 0)
                    safe_send(client.first, recvBuff, pkt_len, 0);
                else{
                    ...
                    safe_send(clientSocket, transBuff, dest, 0);
                }
